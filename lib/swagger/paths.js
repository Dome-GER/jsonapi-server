...
swaggerPaths._getPathOperationObject = options => {
  const pathDefinition = {
    ...
    responses: {
      '200': {
        ...
        schema: {
          title: `${options.resourceName}.base_model`,
          type: 'object',
          required: [ 'jsonapi', 'meta', 'links' ],
          properties: {
            jsonapi: {
              title: `${options.resourceName}.base.jsonapi_model`,
              type: 'object',
              required: [ 'version' ],
              properties: {
                version: {
            ...
            links: {
              title: `${options.resourceName}.base.links_model`,
              type: 'object',
              required: [ 'self' ],
              properties: {
                self: {
  ...
  if (((options.handler === 'search') || (options.handler === 'find')) && !options.relation) {
    ...
    responseShortcut.included = {
      type: 'array',
      items: {
        title: `${options.resourceName}.included_model`,
        type: 'object'
      }
    }
  }
