...
swaggerPaths._getResourceDefinition = resourceConfig => {
  ...
  const resourceDefinition = {
    ...
    properties: {
      ...
      'attributes': {
        title: `${resourceConfig.resource}.attributes_model`,
        type: 'object',
        properties: { }
      },
      'relationships': {
        title: `${resourceConfig.resource}.relationships_model`,
        type: 'object',
        properties: { }
      },
      'links': {
        title: `${resourceConfig.resource}.links_model`,
        type: 'object',
        properties: {
          self: {
            type: 'string'
  ...
  for (const attribute in attributes) {
    ...
    if (joiScheme._notes) {
      swaggerScheme.notes = joiScheme._notes;
    }

    if (joiScheme._tags) {
      swaggerScheme.tags = joiScheme._tags;
    }

    if (joiScheme._examples) {
      swaggerScheme.example = joiScheme._examples[0];
    }
